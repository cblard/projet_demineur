...
ClickableLabel4::ClickableLabel4(QWidget* parent, Qt::WindowFlags f)
    : QLabel(parent) {
}

ClickableLabel4::ClickableLabel4(int abs, int ord, vector<vector<char>> solution, QWidget* parent, Qt::WindowFlags f) : QLabel(parent){
    this->abs=abs;
    this->ord=ord;
...
void ClickableLabel4::mousePressEvent(QMouseEvent* event){
    ...
    if(event->button()==Qt::RightButton){
        if(checkFlag==false){
            this->setPixmap(flag.scaled(23,23,Qt::KeepAspectRatio));
            this->update();
        }
        if(checkFlag==true){
            this->setPixmap(caseVide.scaled(23,23,Qt::KeepAspectRatio));
            this->update();
        }
        checkFlag=!checkFlag;
    }
...
void ClickableLabel4::comparerCases(){
    ...
    if(solution.at(abs).at(ord)=='*'){
        this->setPixmap(bombe.scaled(23,23,Qt::KeepAspectRatio));
        this->update();
        QMessageBox msgBox;
    ...
    if(solution.at(abs).at(ord)==0){
        this->setPixmap(case0.scaled(23,23,Qt::KeepAspectRatio));
        this->update();
    }
    if(solution.at(abs).at(ord)==1){
        this->setPixmap(case1.scaled(23,23,Qt::KeepAspectRatio, Qt::SmoothTransformation));
        this->update();
    }
    if(solution.at(abs).at(ord)==2){
        this->setPixmap(case2.scaled(23,23,Qt::KeepAspectRatio));
        this->update();
    }
    if(solution.at(abs).at(ord)==3){
        this->setPixmap(case3.scaled(23,23,Qt::KeepAspectRatio));
        this->update();
    }
    if(solution.at(abs).at(ord)==4){
        this->setPixmap(case4.scaled(23,23,Qt::KeepAspectRatio));
        this->update();
    }
    if(solution.at(abs).at(ord)==5){
        this->setPixmap(case5.scaled(23,23,Qt::KeepAspectRatio));
        this->update();
    }
    if(solution.at(abs).at(ord)==6){
        this->setPixmap(case6.scaled(23,23,Qt::KeepAspectRatio));
        this->update();
    }
    if(solution.at(abs).at(ord)==7){
        this->setPixmap(case7.scaled(23,23,Qt::KeepAspectRatio));
        this->update();
    }
    if(solution.at(abs).at(ord)==8){
        this->setPixmap(case8.scaled(23,23,Qt::KeepAspectRatio));
        this->update();
    }
}
